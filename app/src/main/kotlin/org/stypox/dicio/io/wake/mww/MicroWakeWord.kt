// Vendored from home-assistant/android microwakeword module (Apache-2.0).
// Patches applied on import:
//   - package repackaged from io.homeassistant.companion.android.microwakeword
//   - Timber replaced with android.util.Log (Dicio doesn't depend on Timber)
// JNI bindings in app/src/main/cpp/MicroWakeWord_jni.cpp FindClass the FQN above —
// keep that string in sync if this file ever moves.
...
class MicroWakeWord(
     ...
     * Safe to call multiple times. Callers MUST invoke this (or use a `use { }` block) —
     * there is no finalizer fallback, so leaking an instance leaks the native handle.
     */
    override fun close() {
        if (nativeHandle != 0L) {
